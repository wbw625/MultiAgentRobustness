...
def start_all(gpus="0,1", port_bias=0):
    ...
    cmd_edited = (
        f"CUDA_VISIBLE_DEVICES={gpu2} /data1/jutj/.conda/envs/fastchat/bin/python -m fastchat.serve.model_worker "
        "--model-path ./models/edited_llama_ike "
        f"--controller http://localhost:{controller_port} "
        f"--port {edited_port} --worker http://localhost:{edited_port}"
    )
    ...
    process_edited = subprocess.Popen(cmd_edited, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    print("Edited已启动")
    time.sleep(35)
...
def kill_process(gpus="0,1", port_bias=0):
    ...
    for port in [edited_port, server_port, llama3_port, controller_port]:
        if is_port_in_use(port):
